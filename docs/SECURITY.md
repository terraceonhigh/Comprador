...
| Privileged helper (`comprador-helper`, `SMAppService.daemon`) | Root | Single largest privilege-escalation surface in the bundle. Unix-socket RPC. Currently bundled for legacy WebDAV cosmetic features only — not on the NFS mount path. **Slated for removal in v0.4.0.** Prioritized in [TODO.md](../TODO.md). |
...
| Writing to phone over MTP | User-level | Bridge accepts NFS WRITEs from the kernel, sends to phone. A path-traversal bug in our `MTPFileSystem.Create()` could let the phone be addressed at unintended paths — but the phone is the *destination*, not the host, so impact is contained to phone-side files. |
